...
export default function MobileSponsorsMarquee() {
  ...
  return (
    <section id="sponsors" className="py-16 bg-ink/95 relative z-10 border-t border-cyan-500/20 overflow-hidden">
      <div className="max-w-md mx-auto px-4 text-center space-y-6 mb-8">
        ...
        <h2 className="text-2xl font-display font-extrabold text-white">
          POWERED BY <span className="text-cyan-400">INDUSTRY LEADERS</span>
        </h2>
      </div>

      {/* Infinite Horizontal Scroll Track */}
      <div className="relative w-full overflow-hidden py-4 bg-cyan-950/20 border-y border-cyan-500/30">
        <div className="flex w-max animate-marquee space-x-6">
          {marqueeSponsors.map((sponsor, idx) => (
            <div
              key={`${sponsor.name}-${idx}`}
              className="flex-shrink-0 w-36 h-20 relative bg-white/5 border border-cyan-500/30 rounded-xl p-3 flex items-center justify-center backdrop-blur-sm"
            >
              <Image
